...
Printer.prototype.selectCharcode = function(charcode) {
  this.buffer.write(commands.CHARCODE['CHARCODE_' + charcode.toUpperCase()]);
  return this;
}
...
Printer.prototype.barcode = function (code, type, width, height, position, font) {
  if (width >= 1 || width <= 255) {
    this.buffer.write(commands.BARCODE_FORMAT.BARCODE_WIDTH + String.fromCodePoint(width));
  }
  if (height >= 2 || height <= 6) {
    this.buffer.write(commands.BARCODE_FORMAT.BARCODE_HEIGHT + String.fromCodePoint(height));
  }
